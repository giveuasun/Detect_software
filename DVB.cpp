...
void cosMake(float freq, float fs, float* pDst, size_t len)
{
...
void sinMake(float freq, float fs, float* pDst, size_t len)
{
	Ipp32f normF = 2 * IPP_PI * freq / fs;
	for (size_t i = 0; i < len; i++)
	{
...
void r2iq(float freq, float fs, float* pSrc, float* pDstI, float* pDstQ, size_t len){
	...
	cosMake(freq, fs, cosData, len); sinMake(freq, fs, sinData, len);
...
void modUW(Ipp32fc* mod)//mod长度卡死26
{
...
int DVB::work(Ipp32f* data, int datalen, float sample_rate, float freq, float Bd, float thread)
{
	...
	//低通滤波器设计
	float Wn = 7.5e+6 / (sample_rate / 2); int N = 35;//滤波器抽头数
	//执行滤波
